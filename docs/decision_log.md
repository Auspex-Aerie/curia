...
### INC-001: Stepwise Council cost summary omits peer-ranking usage
- **date:** 2026-07-13 · **status:** remediated (verified 2026-07-20) · **triggered_by:** Curia-on-Curia MCP dogfood turn `f78e96e6-bbfa-4cbb-8fdd-5b5a0e3ef842` · **docs_updated:** `docs/decision_log.md`, `backend/turn_service.py`, `backend/cost_tracking.py`, `tests/unit/test_cost_tracking.py`, `tests/unit/test_turn_service.py` · **related:** `DEC-015`, `DEC-017`, `DEC-018`
- **symptom:** Completed stepwise turn reported 3 calls / 26,488 tokens although its five successful calls used 40,938 tokens; the paid USD total remained correct because omitted rankings used free models.
- **root_cause:** `TurnService._run_stage3` built the synthetic rankings step without `sum_usage_fields(stage2)`, and nested aggregated `calls` were counted as one by `sum_usage_fields` (`backend/turn_service.py`, `backend/cost_tracking.py`).
- **blast_radius:** Cost/token displays and conversation summaries undercount every completed stepwise Council turn with successful rankings; billing totals can also undercount when ranking models are paid.
- **why_not_caught_earlier:** Unit coverage tested flat step lists and stepwise completion independently, but never asserted cost parity across answer, ranking, and chairman stages.
- **resolution:** `TurnService._run_stage3` now spreads `sum_usage_fields(turn.stage2)` into the synthetic rankings step (`backend/turn_service.py`), and `sum_usage_fields` counts nested aggregated `calls` rather than collapsing them to one (`backend/cost_tracking.py`). Verified 2026-07-20 by `tests/unit/test_cost_tracking.py::TestSumUsageFields::test_summarize_turn_cost_preserves_aggregated_calls` and the stepwise cost-parity assertion in `tests/unit/test_turn_service.py` (`calls == 3` with full token totals across answer, ranking, and chairman).

### INC-002: CodeRAG drops explicit paths and quality-attachment evidence
- **date:** 2026-07-13 · **status:** remediated by `DEC-021` · **triggered_by:** Curia-on-Curia MCP dogfood turn `f78e96e6-bbfa-4cbb-8fdd-5b5a0e3ef842` · **docs_updated:** `docs/decision_log.md`, `backend/rag/chunker.py`, `backend/rag/hybrid.py`, `backend/rag/rerank.py`, `backend/rag/retriever.py`, `tests/unit/test_chunker.py`, `tests/unit/test_hybrid.py`, `tests/unit/test_rerank.py`, `tests/unit/test_retriever.py` · **related:** `DEC-010`, `DIS-001`, `DEF-004`
- **symptom:** A query explicitly naming four repository paths omitted `backend/routes/turns.py`; its final 20-chunk bundle also omitted `backend/run_turn.py`, `backend/execution_quality.py`, and `mcp_arena/quality.py`, leading both surviving Council members to invent the same incorrect `execution_quality` explanation.
- **root_cause:** Chunk/reranker text excluded source paths; the fallback path scan did not normalize punctuation; auxiliary RRF seeds displaced semantic-tail candidates; `jina-reranker-v3` was incorrectly loaded through a pairwise `CrossEncoder`, producing a missing randomly initialized classification head; architectural routing disabled graph expansion; repeated AST parent bodies and prose artifacts consumed final slots.
- **blast_radius:** CodeRAG queries with exact filenames, multi-file flows, or code-style identifiers could omit user-selected evidence, vary across server processes, and overrepresent documentation/tests in the final prompt.
- **why_not_caught_earlier:** Recall evals measured semantic checkpoints on golden symbols/traces but did not assert exact-path preservation, reranker load compatibility/determinism, or uniqueness and source diversity of the final injected bundle.

### DEC-021: Treat explicit code evidence as hard retrieval constraints
- **date:** 2026-07-13 · **status:** accepted · **triggered_by:** `INC-002` dogfood replay and two identical-query reranker processes producing materially different top-20 results · **docs_updated:** `docs/decision_log.md`, `backend/rag/chunker.py`, `backend/rag/hybrid.py`, `backend/rag/rerank.py`, `backend/rag/retriever.py`, `tests/unit/test_chunker.py`, `tests/unit/test_hybrid.py`, `tests/unit/test_rerank.py`, `tests/unit/test_retriever.py` · **related:** `DEC-001`, `DEC-004`, `DEC-010`, `DEF-004` · **resolves:** `INC-002`
- **decision:** Put repository paths into chunk index/rerank text; resolve explicit relative paths (and unique bare filenames) into one protected, query-relevant AST chunk per file; add a lexical lane for code identifiers; extend rather than displace the semantic candidate pool (bounded at Jina v3's 64-document batch); force multi-path questions onto cross-file graph expansion; deduplicate AST parent bodies before and after graph append; and cap prose/test artifacts at 25% for explicit-code queries. Load `jina-reranker-v3` through its required listwise `AutoModel.rerank()` interface, pin remote code/weights to revision `10fb694f…`, and place it on Curia's resolved inference device.
- **rationale:** An exact path or identifier supplied by the user is selection evidence, not a soft semantic hint. Protected recall guarantees those constraints while the semantic, rerank, and graph layers still discover unmentioned supporting code. Source diversification prevents prose relevance from crowding out executable evidence without penalizing documentation-focused queries.
- **impact:** The original dogfood replay now returns all four named files at ranks 1–4, `backend/execution_quality.py` at 13, `backend/run_turn.py` at 19, and graph-expanded `mcp_arena/quality.py` at 23; a second fresh process produced the identical ordering and scores. `DEF-004` remains active for broader index exclusion and conditional-rerank work.

### DEC-022: Make the Observatory rail mode-adaptive, not Council-shaped
- **date:** 2026-07-13 · **status:** accepted · **triggered_by:** post-dogfood Observatory review · **docs_updated:** `docs/decision_log.md`, `frontend/src/deck/inspector.ts`, `frontend/src/deck/participants.ts`, `frontend/src/deck/pulse.ts`, `frontend/src/deck/cost-panel.ts`, `frontend/src/deck/deck.css` · **related:** `PIV-002`, `DEC-014`, `DEC-017`
- **decision:** Split the right rail vertically into **Participants**, **Deliberation pulse**, and **Cost**. Participants stay compact in the rail and expand into a full roster dialog. Pulse uses only mode-valid derived signals: ranking agreement for comparable independent Council answers; adversarial coverage for Fight; refinement continuity for Round Robin; branch coverage for Stacks; progression/reflection coverage for iterative modes. Dependent-chain and unknown modes explicitly decline to infer consensus. Cost exposes independently toggled Current session, Same squad, and Everything in memory series; breakdown is available only when exactly one series is active.
- **rationale:** Persistent rail space should answer who participated, what the current mode's evidence actually supports, and what it cost. A universal “consensus” gauge would be false precision in modes whose later outputs depend on earlier outputs or whose purpose is adversarial disagreement.

### DEF-012: Defer provider/model favicon avatar assets
- **date:** 2026-07-13 · **status:** active · **triggered_by:** `DEC-022` participant roster design · **docs_updated:** `docs/decision_log.md` · **related:** `DEC-022`, `DEF-008`
- **decision:** Ship deterministic initial-based, provider-colored participant avatars now. Defer curated favicon-sized provider/model logos, asset licensing/provenance, light/dark variants, cache/fallback behavior, and catalog-level avatar metadata.
- **revisit_when:** the model catalog exposes stable provider identity metadata or Curia establishes a licensed icon asset pipeline.

### INC-003: Round Robin lineage was executed but unobservable and miscounted
- **date:** 2026-07-14 · **status:** remediated by `DEC-023` · **triggered_by:** CodeRAG Round Robin dogfood turn `763fefc4-5d7b-40a4-8d6a-9f64e7c4de4d` · **docs_updated:** `docs/decision_log.md`, `backend/execution_trace.py`, `backend/prompts/registry.py`, `frontend/src/deck/` · **related:** `DEC-022`, `PIV-002` · **resolves:** participant `4/4` false-success display, “2 handoffs” semantic error, hidden predecessor drafts, duplicated Chair Final
- **symptom:** Observatory presented all internal steps as Council-shaped Answers/Rankings/Verdict, counted failed terminal outcomes as participants, called two non-empty drafts “2 handoffs,” omitted stored `prior_draft` lineage from both Answers and Context, and repeated `chair_final` in Answers and Verdict. The mode composer also embedded the full CodeRAG prompt twice after budgeting, so a reported 13,893-token prompt became 27,298 provider input tokens.
- **root_cause:** Backend modes emitted ad-hoc `metadata.steps`, while Participants, Pulse, Context, Quality, Cost, and the deck independently inferred status and meaning from response text, role-name prefixes, Council stage arrays, or “not pending.” Round Robin appended a turn template containing `user_query` to a per-model prompt that already contained the grounded query.
- **why_not_caught_earlier:** Tests asserted that `prior_draft` reached the next model but not that every consumer could reconstruct the same graph, counts, or final composed prompt. The generic advanced-mode viewer intentionally discarded all step fields except role/model/response.

### DEC-023: Make a canonical execution trace the sole source of run topology and counts
- **date:** 2026-07-14 · **status:** accepted · **triggered_by:** `INC-003`; repeated cross-panel counting drift · **docs_updated:** `docs/decision_log.md`, `backend/execution_trace.py`, `backend/models.py`, `backend/run_turn.py`, `backend/turn_service.py`, `backend/execution_quality.py`, `frontend/src/deck/execution-trace.ts`, `frontend/src/deck/participants.ts`, `frontend/src/deck/pulse.ts`, `frontend/src/deck/turn-context.ts`, `frontend/src/deck/viewers/`, `frontend/src/deck/cost*.ts` · **related:** `DEC-022`, `PIV-001`, `PIV-002` · **resolves:** `INC-003`
- **decision:** Persist `metadata.execution_trace` version 1 as a compact graph over existing payloads. Every node declares stable identity, ordinal, semantic kind/role, model, explicit status, payload reference, predecessor step IDs, input/output artifact IDs, terminal ownership, and failure metadata. The trace publishes canonical attempted/succeeded/failed, participant, draft, refinement, delivery, and final-status counts. Large prompts and responses remain in their existing payload collections and are referenced rather than duplicated. Frontend consumers must use the trace; a version-0 legacy adapter reconstructs old conversations only for backward compatibility.
- **rationale:** Execution truth belongs to orchestration, not six independent UI heuristics. Explicit artifacts and edges make sequential, adversarial, branching, and synthesis dependencies inspectable while giving Quality, Cost, Participants, and Pulse identical semantics. Mode-specific visualization can change without changing the run contract.
- **round_robin presentation:** Replace Council timeline labels with Context → Sequence → Verdict → Quality; show numbered model steps, predecessor input beside revised output, explicit failures/fallback reuse, and trace counts. Terminal chair output belongs only to Verdict. Remove the duplicated grounded query from the per-turn refinement template and instruct refiners to audit—not merely polish—the predecessor.

### DEC-024: Add a low-cost paid squad for reliable dogfood runs
- **date:** 2026-07-14 · **status:** accepted · **triggered_by:** Free-tier rate limits obscuring Round Robin orchestration tests · **docs_updated:** `docs/decision_log.md`, `backend/squads/cheap_pros.json`, `data/model_catalog.yaml` · **related:** `DEC-018`, `DEC-023`
- **decision:** Add **Cheap Pro's**, a four-member paid squad using `openai/gpt-oss-20b`, `qwen/qwen3-coder-30b-a3b-instruct`, `mistralai/mistral-small-3.2-24b-instruct`, and `nvidia/nemotron-3-nano-30b-a3b`, chaired by `deepseek/deepseek-v4-flash`. Record their OpenRouter context limits in the local model catalog; none carries the `free` tag.
- **rationale:** Keep multi-model dogfood runs inexpensive while avoiding free-tier-specific throttling and preserving provider diversity, a code specialist, and a 1M-context synthesis chair. The 2026-07-14 OpenRouter catalog prices the selected models at $0.029–$0.09 per million input tokens and $0.14–$0.27 per million output tokens.

### DEC-025: Make context injection a first-class inspectable workflow
- **date:** 2026-07-14 · **status:** accepted · **triggered_by:** Round Robin and Council context review; prompt tabs obscured stage/recipient meaning and live refresh repeatedly collapsed expanded prompt content · **docs_updated:** `docs/decision_log.md`, `backend/arena.py`, `backend/models.py`, `backend/routes/execution.py`, `frontend/src/deck/injection-workflow.ts`, `frontend/src/deck/viewers/context.ts`, `frontend/src/deck/store.ts`, `frontend/src/deck/pulse.ts` · **related:** `DEC-022`, `DEC-023`, `INC-004`
- **decision:** Replace the Injected context prompt dump with an SVG workflow graph over the RAG-attachment event, traced arena recipients, interstage artifacts, and terminal synthesis. Round Robin renders sequential predecessor handoffs; Council renders independent answers → shared anonymized answer packet → peer rankings → answers+rankings synthesis. Recipient nodes open modals containing only Curia-owned orchestration text and handoff artifacts—never the duplicated RAG body—with a link to the dedicated RAG Retrieval panel. Persist typed `orchestration_text` separately from `prompt_full` for Council and Round Robin stages. Modal and RAG expansion state live in the deck store so polling cannot reset them. Round Robin completion counts are presentation-labeled as calls; a deterministic ≥0.94 lexical-overlap check separately flags near-copy handoffs rather than crediting them as meaningful refinements.
- **rationale:** Injection is a routing event, not another place to dump retrieved text. Operators need to see that RAG was attached, who received the grounded turn, and exactly what Curia inserted between calls; retrieved chunks remain inspectable once in their purpose-built panel. Execution success proves delivery and response, not semantic improvement.
- **council presentation:** Answer nodes carry their final aggregate peer position; ranking nodes carry that evaluator's compact anonymous ballot; the chair carries the aggregate leader entering synthesis, not a fictitious chair rank. The repeated global aggregate block moves out of every model ballot into its own peer-review tab. Native Arena-addition disclosures are keyed in deck state so both open and explicitly closed states survive polling.

### INC-004: Execution detail endpoint collapsed canonical Council ranking nodes
- **date:** 2026-07-14 · **status:** remediated · **triggered_by:** Cheap Pro's Council Code RAG turn `aefe5b1f-c0e0-401e-a396-2c8cb6a8e0fa` · **docs_updated:** `docs/decision_log.md`, `backend/routes/execution.py`, `tests/unit/test_execution_route.py` · **related:** `INC-001`, `INC-003`, `DEC-023`, `DEC-025`
- **symptom:** The turn correctly reported 9 calls and its canonical trace contained 4 answers, 4 rankings, and 1 verdict, but `/messages/{index}/execution` returned only 6 step rows because all peer rankings were represented by one synthetic metadata step.
- **root_cause:** The execution-detail route preferred legacy `metadata.steps` over `metadata.execution_trace`, bypassing the canonical payload references established by `DEC-023`.
- **resolution:** Resolve every execution-detail row from the canonical trace's collection/index source, preserving ordinal, kind, status, predecessors, and terminal ownership. The same turn now returns all 9 nodes with unchanged cost totals.

### INC-005: Fight lineage, quality roles, and viewport disagreed on adversarial topology
- **date:** 2026-07-14 · **status:** remediated by `DEC-026` · **triggered_by:** Cheap Pro's Fight Code RAG turn `bc05e648-0aa0-4269-a3f4-5ac0b8f041da` · **docs_updated:** `docs/decision_log.md`, `backend/execution_trace.py`, `backend/execution_quality.py`, `frontend/src/deck/viewers/council.ts`, `tests/unit/test_execution_trace.py`, `tests/unit/test_execution_quality.py` · **related:** `DEC-023`, `DEC-025`
- **symptom:** The trace claimed every defense received all critiques, including its own, although the runner excludes the defender's critique. Execution Quality searched for obsolete `answer_` / `critique_` / `defense_` prefixes and could miss empty canonical Fight phases. The Observatory collapsed twelve debate steps into duplicate tabs named only `answer`, `critique`, or `defense`.
- **root_cause:** Fight had a canonical trace branch but its parent builder, quality heuristic, and generic advanced-mode renderer were authored from three different role conventions rather than tested against the runner's actual prompt packets.
- **resolution:** Defense parents are exactly own opening + peer critiques; quality uses canonical role names; focused tests cover both. The mode-specific viewport exposes phase, model, status, and exact input artifacts.

### DEC-026: Present Fight as an adversarial exchange, not a flat pipeline
- **date:** 2026-07-14 · **status:** accepted · **triggered_by:** `INC-005` and live Fight dogfood · **docs_updated:** `docs/decision_log.md`, `backend/arena.py`, `backend/prompts/registry.py`, `frontend/src/deck/injection-workflow.ts`, `frontend/src/deck/viewers/council.ts`, `frontend/src/deck/deck.css` · **related:** `DEC-022`, `DEC-023`, `DEC-025` · **resolves:** `INC-005`
- **decision:** Register and persist RAG-free orchestration projections for Fight critique, defense, and chair prompts. Render Injected Context as opening positions → peer critiques → defenses → synthesis. Render Steps with Opening/Critique/Defense phase tabs, model tabs within each phase, and exact predecessor-artifact chips; label self-exclusion explicitly. Do not infer agreement in an adversarial mode.
- **rationale:** A successful Fight is defined by opposition coverage and defended revision, not consensus. Operators must be able to distinguish the response body from Curia's routing text and verify who attacked or defended which artifacts without reading full provider prompts.
- **verification:** Paid Code RAG turn completed 13/13 calls with 32 canonical edges, no failures or summarization, Quality `ok`, 284,744 tokens, and `$0.036461` total cost. Live UI showed 4 openings, 4 critiques, 4 defenses, and 1 chair with RAG-free node modals and no console errors.

### DIS-003: Final two mode smokes execute cleanly but remain generically observable
- **date:** 2026-07-15 · **status:** observed · **triggered_by:** Cheap Pro's Code RAG smoke of the final configured modes · **docs_updated:** `docs/decision_log.md` · **related:** `DEC-023`, `DEC-025`, `INC-006`
- **finding:** **Complex Iterative** conversation `a9d72108-329f-485e-b6d8-9d3e070721cc` completed 5/5 calls (extract → expand twice → chair), Quality `ok`, no failures or summarization, 56,133 tokens, and `$0.009687`. **Complex Questioning** conversation `5fc0e332-6bb3-4630-8748-cf3b05654fae` completed 14/14 calls (4 answers → 4 self-questions → brief → 4 muses → chair), 25 trace edges, Quality `ok`, no failures or summarization, 135,565 tokens, and `$0.022525`. Exact requested files were the first three Code RAG sources in both runs. The live UI produced no console errors.
- **observation:** Both modes still use the generic Pipeline viewport and generic “Predecessor draft / Synthesis packet” graph semantics. Complex Iterative persists no RAG-free orchestration projections; Complex Questioning persists them only for its four opening answers. Complex Questioning's fixed chairman “What I saw” expansion produced 559 words despite the user's 240-word limit, while Complex Iterative stayed within it at 232 words. These are follow-up design inputs, not smoke blockers.
- **implication:** Execution/storage/trace/cost basics are viable in both modes. A future focused pass should add mode-specific orchestration projections and visual vocabulary, and decide how terminal synthesis preserves user output constraints across intermediate briefs.

### INC-006: Complex Iterative counts unused squad members as expected participants
- **date:** 2026-07-15 · **status:** resolved by `DEC-030` · **triggered_by:** Complex Iterative smoke `a9d72108-329f-485e-b6d8-9d3e070721cc` · **docs_updated:** `docs/decision_log.md` · **related:** `DEC-023`, `DIS-003`, `DEC-030`
- **symptom:** A fully successful run reports `participant_expected: 4`, `participant_succeeded: 2`, and no participant failures; the completed Participants rail leaves Mistral and Nemotron as “waiting.”
- **root_cause:** `run_mode_complex_iterative` intentionally assigns only `models[0]` to extract and `models[1]` to expand, while canonical trace construction receives the entire four-model configured squad as its expected participant set.
- **blast_radius:** Complex Iterative squads larger than two look partially attended even when every planned step succeeds; Quality remains `ok`, so counts and status disagree semantically.
- **why_not_caught_earlier:** Canonical trace tests cover lineage but not mode-specific participant eligibility for runners that intentionally use a subset of the configured squad.
- **resolution:** Generalized and resolved by `DEC-030`. The squad-utilization contract makes the assigned-vs-reserved split explicit and computes participant/quality expectations from the assigned set. Phase A corrects the backend counts (canonical-trace `participant_expected` and `stats.arena_models` now read the assigned set, so a quorum Complex Iterative run reports two expected participants, not four); under `require_all` the runner consumes the full squad. Phase C renders the Observatory Participants rail from the assigned set with an explicit reserved section (no perpetual "waiting" on unscheduled members) and surfaces the pre-flight plan in the inspector.

### DEC-027: Keep conversation JSON canonical; project Sessions into reconciled SQLite
- **date:** 2026-07-15 · **status:** accepted · **triggered_by:** Observatory session history outgrowing the turn rail; need for server-side filters, stable pagination, cost metadata, and durable agent-to-human session links · **docs_updated:** `docs/decision_log.md`, `backend/storage_service.py`, `backend/session_catalog.py`, `backend/session_projection.py`, `backend/routes/sessions.py`, `frontend/src/deck/sessions-view.ts`, `frontend/src/deck/session-url.ts` · **related:** `PIV-001`, `PIV-002`
- **decision:** Conversation JSON remains the canonical record. Maintain a recoverable SQLite session catalog as a query projection, updated through the storage mutation boundary with atomic JSON replacement, conversation-keyed striped locking, write intents, monotonic source revisions, and checksums covering the conversation plus turn sidecars. Reconcile the projection at startup and expose keyset-paginated, server-filtered results through `/api/sessions`; retain `/api/conversations` as a bounded compatibility view over the same service.
- **rationale:** A searchable, infinite-loading Sessions page needs indexed queries without creating two competing sources of truth. The projection can be rebuilt after interruption or schema evolution, while canonical records preserve the existing storage contract.
- **impact:** Sessions receives a dedicated full-width page with stable deep links, useful operational/cost metadata, browser-local filtering of loaded rows, and SQL-backed facets and filters. Projection drift is detectable and repairable without rewriting canonical conversations.

### DEC-028: Separate execution topology from exact prompt provenance
- **date:** 2026-07-16 · **status:** accepted · **triggered_by:** Fight defense prompts were executed but their exact injected opening/critique artifacts were represented only as inert prose markers in Context · **docs_updated:** `docs/decision_log.md`, `backend/prompt_provenance.py`, `backend/execution_trace.py`, `frontend/src/deck/injection-workflow.ts`, `frontend/src/deck/turn-context.ts` · **related:** `DEC-023`, `DEC-025`, `DEC-026`
- **decision:** Keep `execution_trace` predecessor edges as the canonical run topology, and add versioned `prompt_provenance` for exact prompt composition. Provenance is a typed sequence of Curia-owned text, grounded-context references, and model-artifact references; logical producer role/model identities resolve centrally into trace step and output-artifact IDs. `orchestration_text` remains a compatibility projection. Retrieved RAG bodies are never duplicated into provenance.
- **rationale:** Topological parents answer “what led to this stage,” while prompt provenance answers “what was actually inserted into this model call.” Conflating them either hides injected artifacts or produces misleading, overly dense graphs. Typed parts make links first-class without frontend parsing of human prose markers.
- **impact:** Fight critique, defense, and chair modals expose exact inline links back to the originating Opening/Critique/Defense reply and a separate RAG event link. The contract is mode-neutral so later Stacks/iterative prompt projections can adopt it without changing the modal or navigation mechanism.

### DEC-029: Establish an independently implemented Apache-2.0 source-tree boundary
- **date:** 2026-07-16 · **status:** accepted · **triggered_by:** public-repository review services require an active MIT, Apache, or GPL license; provenance review found the seed project's source had no explicit license · **docs_updated:** `LICENSE`, `NOTICE`, `LICENSING.md`, `pyproject.toml`, `frontend/package.json`, `README.md`, `CHANGELOG.md`, `CLAUDE.md`, `docs/piv-003-curia-rebrand.md`, `docs/curia-handoff.md`, `docs/decision_log.md` · **related:** `PIV-003`, `DEC-020` · **supersedes:** the PolyForm Shield licensing posture in `PIV-003` and `DEC-020`
- **decision:** Replace remaining inherited implementation and media in the current tree with independently written Curia code, then license releases from the transition commit forward under the Apache License 2.0 SPDX identifier `Apache-2.0`. Ship the unmodified standard license text, a repository-level `NOTICE` identifying Auspex Labs, and a provenance boundary in `LICENSING.md`. Do not claim to relicense historical revisions and do not bulk-add per-file headers. Curia is currently developed in the open; any future separately developed commercial offering does not alter the perpetual terms of versions already released under Apache-2.0.
- **rationale:** Apache-2.0 is a standard OSI-approved permissive license with explicit copyright and patent grants, contribution terms, redistribution notice requirements, and no grant of trade-name, trademark, service-mark, or product-name rights beyond customary attribution. An explicit current-tree rewrite avoids relying on unlicensed upstream expression while retaining honest conceptual credit.
- **impact:** The current implementation has a clear licensing boundary suitable for use, distribution, contribution, and automated repository review. Competing use and commercial forks are permitted. Redistributors must provide the license, mark modified files, retain applicable source notices, and preserve the readable Auspex Labs attribution from `NOTICE`. Historical commits are not retroactively relicensed. A fork may rename its product, but Apache-2.0 does not authorize it to remove applicable origin attribution or present itself using Auspex Labs product identity.

### INC-007: Session-catalog reconciliation crashes on null execution_quality
- **date:** 2026-07-23 · **status:** remediated · **triggered_by:** launching the Observatory against the local conversation corpus; the deck rendered no sessions and every storage endpoint returned HTTP 500 · **docs_updated:** `docs/decision_log.md`, `backend/session_projection.py`, `tests/unit/test_session_catalog.py` · **related:** `DEC-027`
- **symptom:** The Observatory loaded its shell but showed no sessions or conversations. `GET /api/sessions` and `GET /api/conversations` both returned 500.
- **root_cause:** `SessionProjector.project` derived per-assistant quality with `(message.get("metadata") or {}).get("execution_quality", {}).get("severity")`. `dict.get(key, default)` only substitutes the default when the key is absent; an assistant message carrying an explicit `execution_quality: null` returned `None`, and `None.get("severity")` raised `AttributeError`. Because `get_storage_service()` reconciles the catalog on construction — once per request — the single malformed record aborted reconciliation and 500'd every storage-backed endpoint.
- **blast_radius:** Any deployment whose corpus contains one assistant message with a null `execution_quality` loses the entire Sessions/Conversations surface until the process restarts with the fix. One such record existed locally (council conversation `a57eb03f`), so a fresh `main` checkout reproduces it against that corpus.
- **resolution:** Coalesce null to `{}` to match the adjacent `metadata or {}` read and the line's existing `or "unknown"` fallback: `((message.get("metadata") or {}).get("execution_quality") or {}).get("severity") or "unknown"`. Null or absent quality now projects as `"unknown"`. Regression test `test_projection_tolerates_null_execution_quality` reproduces the exact null-quality shape and asserts the session projects as `unknown`/`complete` rather than crashing.
- **why_not_caught_earlier:** Projection tests exercised assistant messages with a populated `execution_quality` dict only; none carried an explicit null, and reconciliation was never tested against a corpus containing a malformed record.

### DEC-030: Squad-utilization contract — declared intent, pre-flight plan, soft-confirm gate
- **date:** 2026-07-20 · **status:** accepted · **triggered_by:** `INC-006` generalized; user review of squad-size intent ambiguity (redundancy vs breadth) and MCP-driven mega-run cost/utilization opacity · **docs_updated:** `docs/decision_log.md`, `backend/squad_plan.py`, `backend/run_turn.py`, `backend/turn_service.py`, `backend/arena.py`, `backend/execution_quality.py`, `backend/routes/turns.py`, `backend/routes/conversations.py`, `backend/models.py`, `mcp_arena/`, `frontend/src/deck/` (B/C), `tests/unit/` · **related:** `INC-006`, `DIS-003`, `DEC-022`, `DEC-023`, `DEC-018`, `PIV-001` · **resolves:** `INC-006`
- **decision:** Introduce a per-turn `squad_policy` that declares how the configured squad maps onto a mode's slots. `quorum` (default) accepts partial success down to a minimum and treats configured members beyond a mode's structural arity as failover **reserves**; `require_all` requires every configured member to produce output, scaling role assignment where a mode would otherwise use a subset. Compute a pre-flight **plan** at the squad-resolution chokepoint before any model call (`backend/run_turn.py` for full turns, `TurnService._create_turn_locked` for stepwise): `models_assigned`, `models_reserved`, `role_map`, exact `projected_calls` (derived from each mode's step formula), the paid/free split of assigned models (from catalog tags), and a stable `plan_fingerprint`. Gate execution with a **soft-confirm** protocol rather than a mid-turn pause: stepwise `create_turn` is already pre-execution, and full-turn `send_message` returns the plan with `requires_confirmation` when `projected_calls` exceeds a configured call-count threshold, executing only when re-invoked with `confirm=<plan_fingerprint>`. The plan surfaces the paid/free split of assigned models for the human's cost judgment; paid-call-weighted gating (per-role call attribution) is a Phase B refinement rather than a paid-presence proxy, since nearly every squad has a paid chair. The gate targets human-absent MCP/agent runs: the live streaming Observatory path, where a human is watching the turn, runs with `enforce_gate=False` and is never blocked. The gate's `agent_notice` always instructs the driving model to obtain user approval before confirming and never to self-approve. Participant and quality expectations read the **assigned** set, not the raw configured squad, so reserves are not counted as failed or missing. `TurnStatus.AWAIT_USER` remains unimplemented and reserved for genuine mid-turn, decoupled/asynchronous human checkpoints (PIV-001 Phase 2); soft-confirm is a forward-compatible subset whose plan artifact and `plan_fingerprint` map onto a future `resume`.
- **rationale:** A large squad expresses one of two undeclared intents — redundancy against unreliable free-tier models, or breadth of voices — yet every mode currently treats a failure identically. Making intent explicit lets `quorum` accept survivors while `require_all` enforces full participation, and dissolves `INC-006` under both policies without a bespoke Complex Iterative patch. Under MCP-driven execution the human is otherwise blind to a turn's call count and paid-model exposure until after it completes; an exact, free pre-flight projection plus a confirm boundary closes that opacity without building the deferred pause/resume machinery. Only Complex Iterative structurally under-uses the squad today (fixed arity 2); the other five modes already consume every configured member, so the contract changes their declared partial-success tolerance without changing their execution.
- **impact:** Complex Iterative under `quorum` reports two assigned participants with the remainder shown as reserves rather than perpetually "waiting"; under `require_all` it exercises the full squad. MCP callers receive a plan and, above threshold or with paid models present, a confirmation requirement that directs them to consult the user before proceeding. Whole-squad modes are unchanged in execution but now declare their partial-success tolerance. Reserves are declared as the failover pool. Phased delivery A–C landed on `feat/squad-utilization-contract`. Follow-ups for threshold config, paid-call gating, and live reserve substitution resolved by `DEC-031`. A dollar-cost projection (`DEF-013`) and a true mid-turn `await_user` checkpoint (PIV-001 Phase 2) remain follow-ups.

### DEF-013: Defer pre-flight dollar-cost projection (persist OpenRouter pricing)
- **date:** 2026-07-20 · **status:** active · **triggered_by:** `DEC-030` pre-flight plan; per-token pricing is not persisted and completion length is unmodeled before a call · **docs_updated:** `docs/decision_log.md` · **related:** `DEC-030`, `DEC-018`
- **decision:** Ship the `DEC-030` plan with exact `projected_calls` and the paid/free split of assigned models only. Defer a dollar `est_cost_usd` band, which would require persisting the OpenRouter `pricing.prompt`/`pricing.completion` fields (already fetched then discarded in `backend/catalog_refresh.py`) into the model catalog schema and assuming a per-role completion-token budget on top of the existing `chars//4` prompt estimate.
- **revisit_when:** operators need an absolute-dollar gate/threshold rather than call-count plus paid-model presence, or the catalog gains persisted pricing for another reason.

### DEC-032: Wire sampling directives; HuggingFace RAG weight distribution
- **date:** 2026-07-25 · **status:** accepted · **triggered_by:** post-DEC-031 backlog — functional directives and explicit RAG model acquisition · **docs_updated:** `docs/decision_log.md`, `README.md`, `RAG_LMSTUDIO.md`, `backend/directives.py`, `backend/openrouter.py`, `backend/sampling.py`, `backend/run_turn.py`, `backend/rag/hf_models.py`, `backend/config.py`, `pyproject.toml` · **related:** `PIV-001` checklist, `DEC-005`, `DEC-011`
- **decision:** (1) Wire `@temp` / `@maxtokens` into every OpenRouter completion for the turn via contextvar sampling overrides set in `run_turn` around `run_full_arena`. (2) Wire `@trace`/`@debug` to attach `metadata.retrieval_trace` (sources, RAG flags, backend/router/rerank config, sampling overrides). (3) Keep `@short`/`@detailed`/`@cite`/`@noexecute` as prompt instructions only; keep `@norag`/`@summarize`/`@tokenbudget`/`@iterations`/`@lastchair`/`@reset` as already functional. (4) Treat `@safe`/`@relaxed` as **non-functional**: parse, warn, do not advertise. (5) Distribute retrieval weights exclusively via HuggingFace Hub (no Git LFS): document model IDs, support `CURIA_HF_HOME`, and ship `uv run curia-prefetch-rag` for offline/pre-warm downloads.
- **rationale:** Operators were told some directives were parsed but dead; sampling overrides are a thin transport change. Safety tiers do not exist — pretending otherwise is worse than a warning. ColBERT/rerank/router weights are multi-GB and already HF-hosted; Git LFS in-repo would bloat the tree and fight the existing `from_pretrained` path.
- **impact:** `@temp`/`@maxtokens` affect all model calls on a turn; `@trace` is inspectable in turn metadata; README directive table matches reality; first-time RAG setup can prefetch without waiting for a live deliberation.

### DEC-033: Auspex-Aerie multi-product Hugging Face presence
- **date:** 2026-07-27 · **status:** accepted · **triggered_by:** HF distribution planning; portfolio is broader than Curia CodeRAG (netflow anomaly, domain lexicon, brand protection, text compression, …) · **docs_updated:** `docs/decision_log.md`, `docs/hf_hub.md` · **related:** `DEC-032`, `PIV-003`, `DEC-020`
- **decision:** Use org slug **`Auspex-Aerie`** everywhere (match GitHub). Structure the Hub as a **lab portfolio**: one **collection per product family** (`curia`, `netflow-anomaly`, `domain-lexicon`, `brand-protection`, `text-compression`, …). Artifact ids are `Auspex-Aerie/<product>-<role>[-vN]`; knobs and stack versions live in cards/`config.json`, not in the name. For Curia v1 Hub publications: **dataset** `curia-router-labels` (public mirror of in-repo router labels) and **config model** `curia-grounding-config` (documented stack recipe citing upstream ColBERT/Jina/MiniLM ids). That config is **not** consumed by Curia runtime today—defaults remain env/`backend/config.py` until an optional remote-recipe load is implemented. Do **not** re-host third-party multi-GB weights unless fine-tuned by Auspex. Org profile and collection blurbs must list multiple lines of work so the first Curia upload does not read as the only product.
- **rationale:** A single `curia-code-rag-*` mega-name both under-sells the lab and overloads one product with stack details. Collections + short role ids scale when netflow/domain/brand/compression models land later without renaming history. Separating “published recipe” from “runtime source of truth” avoids maintainers treating a Hub upload as an instant config switch.
- **impact:** HF planning and upload scripts target `Auspex-Aerie/*`; Curia docs point at `docs/hf_hub.md`. Next implementation step is upload tooling + cards for the two Curia artifacts and org README skeleton.

### DEC-031: Soft-confirm thresholds in frozen config, paid-call gate, live reserve substitution
- **date:** 2026-07-24 · **status:** accepted · **triggered_by:** DEC-030 follow-ups after Phases A–C merged · **docs_updated:** `docs/decision_log.md`, `data/arena_config.yaml`, `backend/frozen_config/schemas.py`, `backend/squad_plan.py`, `backend/arena.py`, `tests/unit/` · **related:** `DEC-030`, `DEC-018`, `INC-006`
- **decision:** (1) Put soft-confirm limits in FREEZE `arena_config.yaml` under `squad_gate.call_threshold` (default 12) and `squad_gate.paid_call_threshold` (default 4); `compute_squad_plan` / `resolve_gate_thresholds` read them. (2) Gate when *either* projected total calls exceed the call threshold *or* projected paid calls (per-role attribution across the exact call list) exceed the paid threshold — not mere paid-model presence, so a free arena with one paid chair stays un-gated at small sizes. Free classification uses catalog `free` tags via `resolve_free_model_ids` when callers omit an explicit set. (3) Wire live reserve substitution in Complex Iterative under `quorum`: if an assigned model fails a step and reserves remain, retry that role with the next unused reserve; record `reserve_substitutions`, `model_failures` (both assigned and failed reserve), and `substituted_for` on the step. Pre-flight gate includes a worst-case failover budget of `min(schedule, reserves)` so approved turns cover substitution traffic. Whole-squad modes still have an empty reserve pool (no substitution). `DEF-013` dollar estimate and mid-turn `AWAIT_USER` remain deferred.
- **rationale:** Operators need tunable confirm policy without code edits; paid call count is the right cost signal when free-tier fan-out is large but paid exposure is the real budget risk; reserves were already declared as failover and only become useful if the runner actually uses them when free models flake; gate must bound failover so soft-confirm approval is not undercut by retries.
- **impact:** MCP/agent large or paid-heavy turns confirm more accurately; Complex Iterative quorum dogfoods recover from single assigned-model failures when spare squad members exist; restart required after editing `squad_gate` (FREEZE).

### DEC-034: Settings + onboarding — three config planes, soft readiness
- **date:** 2026-07-29 · **status:** accepted · **triggered_by:** post-merge product gap; thin settings modal vs rich catalog APIs; DEF-008 revisit · **docs_updated:** `docs/decision_log.md`, `backend/routes/settings.py`, `backend/dependencies.py`, `backend/settings_status.py`, `frontend/src/deck/` · **related:** `DEC-018`, `DEF-008`, `DEC-030`, `PIV-002`
- **decision:** (1) **Three config planes** — *hot* (`data/config.json`: squad composition, chair, default `squad_policy`, theme, `repo_root`); *FREEZE* (`arena_config.yaml` / `model_catalog.yaml` via `/api/catalog/*`, `requires_restart`); *secret/process* (env: OpenRouter key, RAG backends — report presence only; write deferred). (2) **Full-page Settings** at `?page=settings` (Setup / Squad / Repository / Appearance in PR1; Catalog panel closes DEF-008 later). (3) **Default `squad_policy`** persists in hot settings (`quorum` default); per-turn override still wins. (4) **Custom squad** — operators may set `arena_models` + `chairman_model` without a preset (`arena_squad: custom`). (5) **Onboarding** is soft readiness via `GET /api/settings/status` (checklist + banner); never a hard API/MCP gate. (6) Secrets response shape includes write capability fields now (`writable`/`write_supported: false`) so UI can grow without contract churn. (7) No process auto-restart in v1.
- **rationale:** Catalog and squad APIs already exist; the gap is operator surface and a single readiness spine agents and UI share. Soft onboarding keeps MCP dogfood unblocked while making first-run gaps visible.
- **impact:** PR1 ships status API + hot Settings page; secret write and catalog editor remain follow-ups. Partially advances DEF-008 (readiness + composition UI; full catalog CRUD still deferred).

### DEF-014: Defer secret write + FREEZE restart orchestration from Settings UI
...
### DEF-015: Defer non-root WSL install path and tool PATH enforcement
...
- **impact:** Conversation JSON grows a route_decision object per assistant turn when context is prepared. Observatory can surface later; SQLite projection unchanged for now (canonical JSON holds the record). **Also changes retrieval policy:** narrowed multi-hop regex in `route_query_regex` and legacy non-router path; composed path+multi-hop flags (path forces append on; multi-hop sets `graph_trace`); OOD abs-floor category `ood_graph_off` when floors enabled. Not instrumentation-only.

### DIS-010: Truncation telemetry cannot use SentenceTransformer.tokenize
- **date:** 2026-08-03 · **status:** observed · **triggered_by:** review of route-decision instrumentation (N1); pattern of DIS-005/008/009 · **docs_updated:** `docs/decision_log.md`, `backend/rag/route_decision.py` · **related:** `DIS-005`, `DIS-008`, `DIS-009`, `DEC-037`, `DEC-043`
- **finding:** `SentenceTransformer.tokenize()` truncates at `max_seq_length` (256), so `len(ids)` is capped and `truncated = n > 256` is **structurally always False** on the embedding path — the only path where truncation occurs. Measurement was written against a convenient API rather than the quantity (true untruncated length). Same failure class as circular purity / blind-window metrics.
- **implication:** Use HF tokenizer with `truncation=False` (or `encode` without truncate). Never use ST `.tokenize` for truncation detection. Prefer tests that assert long strings set `truncated=True`.

### DIS-011: Standing check — metrics that cannot see their subject
- **date:** 2026-08-03 · **status:** observed · **triggered_by:** four instances: DIS-005 (circular purity), DIS-008/009 (graph-blind k), DIS-010 (always-False truncated) · **docs_updated:** `docs/decision_log.md` · **related:** `DIS-005`, `DIS-008`, `DIS-009`, `DIS-010`, `HYP-003`
- **finding:** Repeated defects share a pattern: a measurement is written against a convenient API/window rather than the quantity under test, so it is **structurally insensitive** when the treatment is a no-op (or always on). Four filed cases in the router program alone.
- **implication:** **Standing check for any new metric/gate:** before shipping, write down the value the metric takes when the treatment is a **no-op** (or fully applied). If that value cannot fail or cannot distinguish arms, do not ship. Prefer tests that force the no-op case. This is process memory — not a one-off fix.

### DIS-012: Helper precision rises when a new consumer upgrades its role
- **date:** 2026-08-03 · **status:** observed · **triggered_by:** four-round review of route-decision PR; path resolution became policy gate · **docs_updated:** `docs/decision_log.md` · **related:** `DIS-011`, `DEC-021`, `DEC-037`, `DEC-043`
- **finding:** `resolve_path_mentions` was written for soft retrieval seeding (`seed_chunks_from_paths`). This PR made it decide path-override and whether the OOD veto is bypassed — and it still backs DEC-021 hard path constraints. Precision requirements jumped; residual findings (U1/U2) concentrated there, not in the instrumentation core.
- **implication:** When attaching a new consumer to an old helper, re-audit the helper as if greenfield for the **stricter** contract. Commit messages about "instrumentation" can hide policy-gate promotions. Pair with DIS-011 (no-op measurement check).

### INC-009: Unparseable module shipped through four review rounds with CI green
- **date:** 2026-08-03 · **status:** resolved · **triggered_by:** Greptile P1 on PR #35 `backend/cli_context.py:73`; confirmed by `py_compile` · **docs_updated:** `docs/decision_log.md`, `backend/cli_context.py`, `.github/workflows/ci.yml` · **related:** `DEC-043`, `DIS-011`
- **symptom:** `backend/cli_context.py` raised `IndentationError: unexpected indent` on import — a `for` header and its first body statement collapsed onto one line, leaving the next line over-indented. The CLI could not run at all.
- **root_cause:** Introduced in `4ef4af0` (first commit of the route-decision PR) and never executed. Nothing imports `backend.cli_context` — it is a standalone entry point with no console-script registration — so `pytest tests/unit` never loaded it. CI ran only the test suite, with no lint or compile step, so 416 green tests coexisted with a module that does not parse.
- **blast_radius:** CLI-only; no runtime or API path affected. Survived four external review rounds and four implementer fix rounds.
- **why_not_caught_earlier:** Reviewer read the changed-file *list* but not this file; the suite cannot fail on a module it never imports. Both human and automated checks were blind to unimported code — the same shape as `DIS-011` (a check that cannot observe part of the system reports success for it).
- **remediation:** Fixed the loop. Added a `compileall` step over `backend mcp_arena RAGRouter` to `.github/workflows/ci.yml`, ahead of the unit tests, so unimported modules are covered by parse at minimum.
- **follow_up:** Consider promoting the compile step to a lint (`ruff`) pass; `compileall` catches syntax only, not unused/undefined names in unimported modules.
