...
- **decision:** Use org slug **`Auspex-Aerie`** everywhere (match GitHub). Structure the Hub as a **lab portfolio**: one **collection per product family** (`curia`, `netflow-anomaly`, `domain-lexicon`, `brand-protection`, `text-compression`, …). Artifact ids are `Auspex-Aerie/<product>-<role>[-vN]`; knobs and stack versions live in cards/`config.json`, not in the name. For Curia v1 uploads: first-party **dataset** `curia-router-labels` and **config model** `curia-grounding-config` (recipe citing upstream ColBERT/Jina/MiniLM ids). Do **not** re-host third-party multi-GB weights unless fine-tuned by Auspex. Org profile and collection blurbs must list multiple lines of work so the first Curia upload does not read as the only product.
- **rationale:** A single `curia-code-rag-*` mega-name both under-sells the lab and overloads one product with stack details. Collections + short role ids scale when netflow/domain/brand/compression models land later without renaming history.
