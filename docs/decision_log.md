...
- **impact:** Complex Iterative under `quorum` reports two assigned participants with the remainder shown as reserves rather than perpetually "waiting"; under `require_all` it exercises the full squad. MCP callers receive a plan and, above threshold or with paid models present, a confirmation requirement that directs them to consult the user before proceeding. Whole-squad modes are unchanged in execution but now declare their partial-success tolerance. Reserves are declared as the failover pool; live substitution of a reserve for a failed assigned model is intent, not yet wired, and is a follow-up. Phased delivery: A (backend contract + Complex Iterative + quality counts), B (MCP plan surface + soft-confirm), C (Observatory assigned/reserved roster + plan preview) — all three phases landed on `feat/squad-utilization-contract`. A dollar-cost projection (`DEF-013`), paid-call-weighted gating, threshold-to-frozen-config wiring, live reserve substitution, and a true mid-turn `await_user` checkpoint (PIV-001 Phase 2) remain follow-ups.
