...
- **resolution:** Generalized and resolved by `DEC-030`. The squad-utilization contract makes the assigned-vs-reserved split explicit and computes participant/quality expectations from the assigned set. Phase A corrects the backend counts (canonical-trace `participant_expected` and `stats.arena_models` now read the assigned set, so a quorum Complex Iterative run reports two expected participants, not four); under `require_all` the runner consumes the full squad. The Observatory Participants rail — the original visible "waiting" symptom — still reads the full configured squad until Phase C renders the assigned/reserved split.
