...
- **decision:** (1) Put soft-confirm limits in FREEZE `arena_config.yaml` under `squad_gate.call_threshold` (default 12) and `squad_gate.paid_call_threshold` (default 4); `compute_squad_plan` / `resolve_gate_thresholds` read them. (2) Gate when *either* projected total calls exceed the call threshold *or* projected paid calls (per-role attribution across the exact call list) exceed the paid threshold — not mere paid-model presence, so a free arena with one paid chair stays un-gated at small sizes. (3) Wire live reserve substitution in Complex Iterative under `quorum`: if an assigned model fails a step and reserves remain, retry that role with the next unused reserve; record `reserve_substitutions`, `model_failures`, and `substituted_for` on the step. Whole-squad modes still have an empty reserve pool (no substitution). `DEF-013` dollar estimate and mid-turn `AWAIT_USER` remain deferred.
- **rationale:** Operators need tunable confirm policy without code edits; paid call count is the right cost signal when free-tier fan-out is large but paid exposure is the real budget risk; reserves were already declared as failover and only become useful if the runner actually uses them when free models flake.
